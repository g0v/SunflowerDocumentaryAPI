...
var options = {
    ...
    port: 443,
    path: '/feeds/list/1enjDLyGzghZt2p27ZG3kaEt2bRxdGzpQ9bEvKmombr4/1/public/values?v=3.0&alt=json',
    method: 'GET',
    headers: {
...
exports.init = function (db) {
    ...
    console.log(dataSheetConfig.sheetArray.length);

    for (var i = 0; i < dataSheetConfig.sheetArray.length; i++)
    {
        for (var k = 1; k <= dataSheetConfig.sheetArray[i].listNum; k++) {
            options.path = '/feeds/list/' + dataSheetConfig.sheetArray[i].sheetID + '/' + k +'/public/values?v=3.0&alt=json';

            console.log(dataSheetConfig.sheetArray[i].sheetID + ' Sheet Num:' + k);
